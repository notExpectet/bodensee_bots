...
void TorFinal()
{
	...
	if ((Abweichung >= TorWinkel-5) && (Abweichung <= TorWinkel+5))
	{
...
task main();
{
	...
	while (true)
	{
		...
		if ((BallDa == true) && (TorOK == true))
		{
			...
			if (Ult1 < 100/2)
			{
				TorWinkel = atan2(100/2-Ult1, Ult2);
			}
			if (Ult1 > 100/2)
			{
				//TorWinkel = (Ulthinten*0.2);
				TorWinkel = -atan2(Ult1-100/2, Ult2);
			}
